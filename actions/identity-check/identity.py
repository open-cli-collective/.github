...
def validate(manifest_path: str, working_dir: str) -> list[str]:
    ...
    # --- .goreleaser (always: binary + archive templates) ---
    gor_path = os.path.join(working_dir, m["goreleaser_config"])
    if not os.path.isfile(gor_path):
        return [f"goreleaser_config not found: {gor_path}"]
    gor = _load_yaml(gor_path)

    builds = gor.get("builds", []) or []
    binaries = {b.get("binary") for b in builds if b.get("binary")}
    if binaries and binaries != {m["binary"]}:
        errors.append(f"goreleaser builds[].binary {sorted(binaries)} != manifest binary '{m['binary']}'")
    ...
    # --- homebrew cask (declared-channel: a declared cask MUST have a native block) ---
