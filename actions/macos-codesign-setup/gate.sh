#!/usr/bin/env bash
# Pure all-or-none decision for macos-codesign-setup. Reads the four cert inputs
# (P12/PASSWORD/CN/LEAF_SHA) and signals via exit code
# (so any ::error:: annotation can go to stdout, where the Actions runner renders it — a
# captured stdout value would hide it):
#   0  = enabled  (all four cert secrets present)
#   10 = disabled (no secrets — the unsigned / opt-out path)
#   1  = invalid  (1-3 of 4 set; prints a ::error:: annotation)
